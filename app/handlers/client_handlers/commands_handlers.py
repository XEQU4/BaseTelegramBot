...
async def command_start_handling(message: Message, state: FSMContext) -> None:
    ...
    text1 = t("greet_client", lang=lang, name=message.from_user.full_name)
    text2 = t("greet_reply", lang=lang)
    await message.answer(text=text1, reply_markup=IKB.client_menu(lang=lang))
    await message.answer(text=text2, reply_markup=RKB.client_menu(lang=lang))
